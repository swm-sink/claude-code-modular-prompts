#!/usr/bin/env python3
"""
Agent V6: Reference Integrity Validator
...
class ReferenceIntegrityValidator:
        ...
        for base_dir in [modules_dir, system_dir, prompt_eng_dir, domain_dir]:
            if base_dir.exists():
                for file_path in base_dir.rglob('*.md'):
                    ...
                    existing_modules.add(str(rel_path))
        ...
        for cmd_file in commands_dir.glob('*.md'):
            ...
            try:
                ...
                for ref in module_refs:
                    ...
                    self.results['module_usage_count'][ref] += 1
                    self.results['commands_to_modules'][cmd_file.name].append(ref)
                    
                    # Check if module exists
                    if ref not in existing_modules:
        ...
        for module_file in modules_dir.rglob('*.md'):
            ...
            try:
                ...
                for ref in module_refs:
                    self.results['total_references'] += 1
                    self.results['module_usage_count'][ref] += 1
                    self.results['modules_to_modules'][str(rel_path)].append(ref)
    ...
    def extract_module_references(self, content):
        ...
        # Pattern 4: modules/path/to/module.md in text
        ...
        for ref in references:
            ...
            if not ref.startswith(('modules/', 'system/', 'prompt_eng/', 'domain/')):
                # Try to infer the correct path
                if any(x in ref for x in ['quality', 'security', 'context', 'session', 'git']):
                    ref = f"system/{ref}"
                elif any(x in ref for x in ['patterns', 'development', 'meta']):
                    ref = f"modules/{ref}"
                else:
                    ref = f"modules/{ref}"
    ...
    def get_reference_pattern(self, ref):
        ...
        if ref.startswith('modules/patterns/'):
            return 'pattern_module'
        elif ref.startswith('modules/development/'):
            return 'development_module'
        elif ref.startswith('modules/meta/'):
            return 'meta_module'
        elif ref.startswith('system/quality/'):
            return 'quality_module'
        elif ref.startswith('system/security/'):
        ...
        try:
            ...
            if arch_match:
                ...
                for cmd_name, module_ref in declared_mappings:
                    ...
                    if module_ref not in existing_modules:
                        self.results['broken_references'].append({
                            'source': 'CLAUDE.md',
    ...
    def generate_report(self):
        ...
        if self.results['orphaned_modules']:
            ...
            for module in sorted(self.results['orphaned_modules']):
