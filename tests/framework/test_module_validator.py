...
class TestModuleValidator:
    ...
    def test_module_token_budget(self, modules_dir):
        """Test that modules stay within 2k token budget."""
        # Rough approximation: 1 token ≈ 4 characters
        max_chars = 2000 * 4  # 8k characters for 2k tokens
        ...
        for module_file in modules_dir.rglob("*.md"):
            ...
            assert char_count <= max_chars, \
                f"Module {module_file.relative_to(modules_dir)} exceeds token budget ({char_count} chars)"
                ...
                for existing_module, existing_purpose in module_purposes.items():
                    if existing_module != module_file:
                        ...
                        if overlap > 10:  # If more than 10 words overlap
                            pytest.warning(f"Potential redundancy between {module_file.name} and {existing_module.name}")
    ...
    def test_module_has_metadata(self, modules_dir):
        ...
        for module_file in modules_dir.rglob("*.md"):
            ...
            assert "<metadata>" in content or "## Metadata" in content or "name:" in content, \
