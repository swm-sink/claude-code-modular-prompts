#!/bin/bash
# Test for Task 8: Move tests/test_agent_integration_system.sh to archive
# TDD: RED-GREEN-REFACTOR-INTEGRATION

set -e

echo "=== Task 8 TDD Test Suite ==="
echo "Testing: Move tests/test_agent_integration_system.sh to archive"
echo ""
...
ARCHIVE_DIR=".archive-integration-approach/tests"
...
test_content_preserved() {
    if [ -f "$TARGET_FILE" ]; then
        ...
        if [ "$ACTUAL_SIZE" -gt 100 ]; then
            ...
            echo "❌ Test 4 FAILED: File seems empty or corrupted"
            return 1
        fi
    else
...
# Run all tests
echo "Running Task 8 validation tests..."
echo "================================="
...
echo "================================="
if [ $FAILED -eq 0 ]; then
    echo "✅ ALL TESTS PASSED - Task 8 complete!"
    exit 0
else
    echo "❌ $FAILED tests failed - Task 8 validation failed"
    exit 1
fi